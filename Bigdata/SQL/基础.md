...
case
        t.operator
        when '>' then if(
            t.left_operand_value > t.right_operand_value,
            'true',
            'false'
        )
        when '=' then if(
...
case
                t1.left_operand
                when t2.name then t2.value
            end as left_operand_value,
            operator,
            t1.right_operand,
case
                t1.right_operand
                when t3.name then t3.value
            end as right_operand_value
        from Expressions t1
            left join (
              -- 确定left_operand的值
                select name,
                    value
                from Variables
            ) t2 on t1.left_operand = t2.name
            left join (
              -- 确定right_operand的值
                select name,
                    value
                from Variables
            ) t3 on t1.right_operand = t3.name
    ) t
```
