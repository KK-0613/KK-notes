...
set
    sex = if(sex = 'm', 'f', 'm');
-- if(e1,e2,e3) 若e1为true则输出e2，否则输出e3
```
...
-- 使用'% DIAB1%'而不是'%DIAB1%'，是因为I类糖尿病的代码总是包含前缀DIAB1
```
...
with t as(
    ...
    from
        Employees t1
        join Salaries t2 on t1.employee_id = t2.employee_id
)
select employee_id from Employees
where employee_id not in(select * from t)
...
where p_id is null --父节点为空

union all
--叶子结点
select id,'Leaf' as type from tree 
where id not in (select p_id from tree where p_id is not null)  --叶子结点id不能出现在父节点列
and id not in (select id type from tree where p_id is null) --叶子结点不能是根结点
)

select id,'Inner' as type from  tree
where id not in(select id from t) --既不是叶子结点，也不是根结点的结点是内部结点
union all 
select id,type
from t
```
...
select ifnull(
    ...
    from 
        (select *,dense_rank() over(order by salary desc) rn from Employee) t 
    where t.rn = 2)
,null) as SecondHighestSalary

-- 此处的ifnull函数的第一个参数的意思是：若select查询语句查询不到任何东西则该查询为null
```
...
```mysql
-- 从SalesPerson中排除是'RED'公司的订单的销售人员id
select name
from SalesPerson
where sales_id not in (
        -- 从订单表中找出是'RED'公司的订单的销售人员id
...
from (
        -- 按customer_number分组并计算每位顾客的订单数，并按照订单数降序排序，最后找出订单数最多的顾客
...
from (
        -- 2.用开窗函数找出所有订单数量最多的顾客的customer_number
        select customer_number,
            rank() over(
...
select t1.stock_name,
    t2.sell_price - t1.buy_price as capital_gain_loss
from (
        select stock_name,
            sum(price) buy_price
        from Stocks
...
```mysql
select t1.name,
    ifnull(t2.travelled_distance, 0) as travelled_distance
from Users t1
    left join (
      -- 每位旅行者的x cheng
        select user_id,
...
order by t2.travelled_distance desc,
    t1.name asc
```
